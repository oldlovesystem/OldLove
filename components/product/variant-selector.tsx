...
export function VariantSelector({
  ...
  return options.map((option) => (
    <form key={option.id}>
      <dl className="mb-8">
        <dt className="mb-4 text-sm font-bold uppercase tracking-wide">{option.name}</dt>
          ...
          {option.values.map((value) => {
            ...
            return (
              <button
                ...
                className={clsx(
                  'flex min-w-[48px] items-center justify-center rounded-full border bg-neutral-100 px-3 py-3 text-sm ',
                  {
