...
export function ProductDescription({ product }: { product: Product }) {
  ...
  const fetchDeliveryAvailability = async () => {
    try {
      const response = await fetch(`https://cancelorder.vercel.app/check-pincode/${postcode}`);
      const data = await response.json();
      
      if (data.delivery_available === "yes") {
        setEstimatedDelivery("Delivery Available: Yes");
      } else {
        setEstimatedDelivery("Delivery Available: No");
      }
    } catch (error) {
      console.error("Error checking delivery availability:", error);
      setEstimatedDelivery("Error checking delivery availability");
    }
  };
  const dropdownItems = [
    {
  ...
  <label className="mb-2 block text-sm">Enter your postal code for delivery availability:</label>
  <div className="flex w-full items-center space-x-2">
    ...
    />
    <button
      onClick={fetchDeliveryAvailability}
